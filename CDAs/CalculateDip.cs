...
public partial class CalculateDip
{
	...
	.RestoreValue("CALCULATE_DIP_OUTPUT_ATTRIBUTE", a => a.FullName, a => Project.ActiveProject.Design.Attributes.GetAttributeOrThrow(a));
		...
		customDesignAction.ApplyAction += (s, e) =>
		{
			...
			{
				var dip = CalculateDipRadians(tri.Data, range1, range2 * Math.PI / 180, debugging, DEBUGGING_LIMIT);
	...
	public static double? CalculateDipRadians(TriangleMesh tm, double dipRange1, double dipRange2, List<DipDebugInfo> debugging = null, int debugging_limit = DEBUGGING_LIMIT)
	{
		...
		if (totalArea < 1e-9) // Avoid division by zero with a small tolerance
		{
	...
	public static double GetDip(TriangleMeshTriangle tri)
	{
		Vector3D normal = tri.Normal;
		Vector3D up = Vector3D.UpVector; // Assuming Vector3D.UpVector is available
		...
		// Calculate the angle between the normal and the up vector in radians
		double angleRadians = System.Math.Acos(dot);
		return angleRadians;
	}
